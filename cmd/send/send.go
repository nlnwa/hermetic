...
└── /kommuner_2023-20230611002730-0036-veidemann-contentwriter-568c6f8545-frvcm
    ...
    └── /checksum_transferred.md5
... etc`)
	if markRootPathRequiredError := command.MarkFlagRequired(transferTopicFlagName); markRootPathRequiredError != nil {
		panic(fmt.Sprintf("failed to mark flag %s as required, original error: '%s'", transferTopicFlagName, markRootPathRequiredError))
	}
